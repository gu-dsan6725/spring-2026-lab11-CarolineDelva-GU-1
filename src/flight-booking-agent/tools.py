"""Tools for Flight Booking Agent - Booking management operations."""
...
def check_availability(
    ...
    logger.info(
        f"Tool called: check_availability(flight_id={flight_id})"
    )
    try:
...
def reserve_flight(
    ...
    logger.info(
        f"Tool called: reserve_flight("
        f"flight_id={flight_id}, passengers={len(passengers)})"
    )
    logger.debug(
        f"Passengers: {passengers}, Requested seats: {requested_seats}"
    )
    try:
        reservation = get_db_manager().create_reservation(
            flight_id, passengers, requested_seats
        )
        logger.debug(
            f"Reservation result:\n"
            f"{json.dumps(reservation, indent=2)}"
        )
...
def confirm_booking(
    ...
    logger.info(
        f"Tool called: confirm_booking(booking_number={booking_number})"
    )
    try:
        confirmation = get_db_manager().confirm_booking(booking_number)
        logger.debug(
            f"Confirmation result:\n"
            f"{json.dumps(confirmation, indent=2)}"
        )
...
def process_payment(
    ...
    logger.info(
        f"Tool called: process_payment("
        f"booking_number={booking_number}, "
        f"payment_method={payment_method})"
    )
    logger.debug(f"Payment amount: {amount}")
    try:
        payment_result = get_db_manager().process_payment(
            booking_number, payment_method, amount
        )
        logger.debug(
            f"Payment result:\n"
            f"{json.dumps(payment_result, indent=2)}"
        )
...
def manage_reservation(
    ...
    logger.info(
        f"Tool called: manage_reservation("
        f"booking_number={booking_number}, action={action})"
    )
    logger.debug(f"Reason: {reason}")
    try:
        db_manager = get_db_manager()
        if action == "view":
            booking_details = db_manager.get_booking_details(
                booking_number
            )
            logger.debug(
                f"Booking details:\n"
                f"{json.dumps(booking_details, indent=2)}"
            )
            ...
            cancellation_result = db_manager.cancel_booking(
                booking_number, reason
            )
            logger.debug(
                f"Cancellation result:\n"
                f"{json.dumps(cancellation_result, indent=2)}"
            )
            ...
            error_msg = (
                ...
                f"Supported actions: view, cancel"
            )
            logger.warning(error_msg)
            return json.dumps({"error": error_msg})
