...
def _insert_seed_data(
    ...
    if cursor.fetchone()[0] == 0:
        flight_data = [
            (
                1, "UA101", "United", "SF", "NY",
                "2025-11-15 08:00", "2025-11-15 16:30",
                330, 250.00, 85, "B737",
            ),
            (
                2, "AA202", "American", "SF", "NY",
                "2025-11-15 10:15", "2025-11-15 18:45",
                330, 280.00, 45, "A320",
            ),
            (
                3, "DL303", "Delta", "SF", "NY",
                "2025-11-15 14:30", "2025-11-15 23:00",
                330, 220.00, 120, "B757",
            ),
            (
                4, "UA104", "United", "SF", "LA",
                "2025-11-16 07:00", "2025-11-16 08:30",
                90, 120.00, 95, "B737",
            ),
        ]
    ...
    if cursor.fetchone()[0] == 0:
        ...
        conn.executemany(
            ...
            (booking_number, flight_id, total_price, status, created_at, confirmed_at)
            VALUES (?, ?, ?, ?, ?, ?)
        """,
            booking_data,
        )
        ...
        conn.executemany(
            ...
            (booking_id, passenger_name, email, seat_number)
            VALUES (?, ?, ?, ?)
        """,
            passenger_data,
        )
        ...
        conn.executemany(
            ...
            (booking_id, amount, status, payment_method, transaction_id, processed_at)
            VALUES (?, ?, ?, ?, ?, ?)
        """,
            payment_data,
        )
        ...
        conn.executemany(
            ...
            (flight_id, seat_row, seat_column, status)
            VALUES (?, ?, ?, ?)
        """,
            seat_data,
        )
...
class BookingDatabaseManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Initializing BookingDatabaseManager with db_path: {db_path}"
        )
        self.init_database()
    ...
    def get_flight_availability(
        ...
        with self.get_connection() as conn:
            cursor = conn.execute(
                """
                SELECT f.flight_number, f.airline, f.departure_city,
                       f.arrival_city, f.departure_time, f.available_seats,
                       f.price
                FROM flights f
                WHERE f.id = ?
            """,
            ...
            logger.info(
                f"Flight availability retrieved: {row[0]}, "
                f"available_seats={row[5]}"
            )
            ...
            return {
                ...
                "availability_status": (
                    "Available" if row[5] > 0 else "Sold Out"
                ),
            }
    ...
    def create_reservation(
        ...
        logger.info(
            f"Creating reservation for flight_id: {flight_id}, "
            f"passengers: {len(passengers)}"
        )
        with self.get_connection() as conn:
            cursor = conn.execute(
                "SELECT price, available_seats FROM flights WHERE id = ?",
                (flight_id,),
            )
            ...
            if available_seats < num_passengers:
                logger.warning(
                    f"Insufficient seats: requested={num_passengers}, "
                    f"available={available_seats}"
                )
                raise ValueError(
                    f"Not enough seats available. "
                    f"Requested: {num_passengers}, "
                    f"Available: {available_seats}"
                )
            ...
            logger.info(
                f"Generated booking_number: {booking_number}, "
            ...
            cursor = conn.execute(
                """
                INSERT INTO bookings
                (booking_number, flight_id, total_price, status)
                VALUES (?, ?, ?, 'pending')
            """,
    ...
    def confirm_booking(
        ...
        with self.get_connection() as conn:
            ...
            if current_status != "pending":
                logger.warning(
                    f"Cannot confirm booking {booking_number}, "
                    f"status: {current_status}"
                )
                raise ValueError(
                    f"Booking {booking_number} cannot be confirmed. "
    ...
    def process_payment(
        ...
        logger.info(
            f"Processing payment for booking: {booking_number}, "
        ...
        with self.get_connection() as conn:
            ...
            payment_amount = (
                amount if amount is not None else float(total_price)
            )

            transaction_id = f"TXN{uuid.uuid4().hex[:8].upper()}"
            processed_time = datetime.now().isoformat()
            logger.info(
                f"Payment transaction created: {transaction_id}, "
            ...
            conn.commit()
            logger.info(
                f"Payment completed: booking={booking_number}, "
        ...
        with self.get_connection() as conn:
            cursor = conn.execute(
                ...
                SELECT b.id, b.booking_number, b.flight_id, b.total_price,
                       ...
                       f.flight_number, f.airline, f.departure_city,
            ...
            for p_row in passenger_cursor.fetchall():
                passengers.append(
                    {"name": p_row[0], "email": p_row[1], "seat": p_row[2]}
                )
    ...
    def cancel_booking(
        ...
        logger.info(
            f"Cancelling booking: {booking_number}, reason: {reason}"
        )
        with self.get_connection() as conn:
            cursor = conn.execute(
                """
            ...
            if current_status == "cancelled":
                logger.warning(
                    f"Booking already cancelled: {booking_number}"
                )
                raise ValueError(
                    f"Booking {booking_number} is already cancelled"
                )

            # 80% refund
            refund_amount = float(total_price) * 0.8
            ...
            conn.commit()
            logger.info(
                f"Booking cancelled: {booking_number}, "
                f"refund_amount: {refund_amount}, "
                f"seats_freed: {num_seats}"
            )
