...
def _insert_seed_data(
    ...
    seed_data = [
        ...
        (
            5, "AA205", "American", "NY", "SF",
            "2025-11-17 09:00", "2025-11-17 12:30",
            330, 260.00, 78, "A321",
        ),
        (
            6, "DL306", "Delta", "LA", "NY",
            "2025-11-18 11:00", "2025-11-18 19:30",
            330, 240.00, 92, "B757",
        ),
    ]
...
class FlightDatabaseManager:
    ...
    def search_flights(
        ...
        logger.info(
            f"Searching flights: {departure_city} -> {arrival_city}, "
    ...
    def get_recommendations(
        ...
        logger.info(
            f"Getting recommendations: max_price={max_price}, "
            f"airlines={preferred_airlines}"
        )
        with self.get_connection() as conn:
                ...
                recommendations.append(
                    {
                        ...
                        "recommendation_score": min(
                            100,
                            int((max_price - float(row[8])) / max_price * 100),
                        ),
                    }
                )
    ...
    def create_trip_plan(
        ...
        logger.info(
            f"Creating trip plan: {departure_city} -> {arrival_city}, "
            f"date: {departure_date}, budget: {budget}"
        )
        with self.get_connection() as conn:
            cursor = conn.execute(
                """
