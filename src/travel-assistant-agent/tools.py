...
def search_flights(
    ...
    try:
        flights = get_db_manager().search_flights(
...
def check_prices(
    ...
    try:
        ...
        logger.debug(
            f"Flight details result:\n"
            f"{json.dumps(flight_details, indent=2)}"
        )
...
def get_recommendations(
    ...
    logger.info(
        f"Tool called: get_recommendations("
        f"max_price={max_price}, "
        f"preferred_airlines={preferred_airlines})"
    )
    try:
        recommendations = get_db_manager().get_recommendations(
...
def create_trip_plan(
    ...
    logger.info(
        f"Tool called: create_trip_plan("
        f"departure_city={departure_city}, "
        f"arrival_city={arrival_city}, "
        f"departure_date={departure_date})"
    )
    ...
    try:
        ...
        outbound_flights = db_manager.search_flights(
            departure_city, arrival_city, departure_date
        )
        return_flights = []

        if return_date:
            return_flights = db_manager.search_flights(
